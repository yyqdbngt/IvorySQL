...
CREATE TABLE num_exp_sub (id1 int4, id2 int4, expected number(210,10));
CREATE TABLE num_exp_div (id1 int4, id2 int4, expected number(210,10));
CREATE TABLE num_exp_mul (id1 int4, id2 int4, expected number(210,10));
CREATE TABLE num_exp_sqrt (id int4, expected number(210,10));
CREATE TABLE num_exp_ln (id int4, expected number(210,10));
CREATE TABLE num_exp_log10 (id int4, expected number(210,10));
CREATE TABLE num_exp_power_10_ln (id int4, expected number(210,10));

CREATE TABLE num_result (id1 int4, id2 int4, result number(210,10));
...
-- ******************************
-- * Now check the behaviour of the NUMBER type
-- ******************************
...
-- ******************************
DELETE FROM num_result;
INSERT INTO num_result SELECT id, 0, LOG(number '10', ABS(val)::number)
    FROM num_data
    WHERE val != '0.0';
SELECT t1.id1, t1.result, t2.expected
...
-- ******************************
DELETE FROM num_result;
INSERT INTO num_result SELECT id, 0, POWER(number '10', LN(ABS(round(val,200))))
    FROM num_data
    WHERE val != '0.0';
SELECT t1.id1, t1.result, t2.expected
...
-- ******************************
-- number AVG used to fail on some platforms
SELECT AVG(val) FROM num_data;
SELECT STDDEV(val) FROM num_data;
SELECT VARIANCE(val) FROM num_data;

-- Check for appropriate rounding and overflow
CREATE TABLE fract_only (id int, val number(4,4));
...
CREATE TABLE ceil_floor_round (a number);
...
-- number and float8 versions of the function in this file.
...
-- normal operation
CREATE TABLE width_bucket_test (operand_num number, operand_f8 float8);
...
SELECT '' AS to_char_24, to_char('100'::number, 'FM999.9');
SELECT '' AS to_char_25, to_char('100'::number, 'FM999.');
SELECT '' AS to_char_26, to_char('100'::number, 'FM999');
...
SELECT '' AS to_number_14, to_number('12.34');
...
CREATE TABLE num_input_test (n1 number);
...
select 999999999999999999999::number/1000000000000000000000;
select div(999999999999999999999::number,1000000000000000000000);
select mod(999999999999999999999::number,1000000000000000000000);
select div(-9999999999999999999999::number,1000000000000000000000);
select mod(-9999999999999999999999::number,1000000000000000000000);
select div(-9999999999999999999999::number,1000000000000000000000)*1000000000000000000000 + mod(-9999999999999999999999::number,1000000000000000000000);
select mod (70.0,70) ;
select div (70.0,70) ;
select 70.0 / 70 ;
...
--
select * from generate_series(0.0::number, 4.0::number);
select * from generate_series(0.1::number, 4.0::number, 1.3::number);
select * from generate_series(4.0::number, -1.5::number, -2.2::number);
-- Trigger errors
select * from generate_series(-100::number, 100::number, 0::number);
select * from generate_series(-100::number, 100::number, 'nan'::number);
select * from generate_series('nan'::number, 100::number, 10::number);
select * from generate_series(0::number, 'nan'::number, 10::number);
-- Checks maximum, output is truncated
select (i / (10::number ^ 131071))::number(1,0)
	from generate_series(6 * (10::number ^ 131071),
			     9 * (10::number ^ 131071),
			     10::number ^ 131071) as a(i);
-- Check usage with variables
select * from generate_series(1::number, 3::number) i, generate_series(i,3) j;
select * from generate_series(1::number, 3::number) i, generate_series(1,i) j;
select * from generate_series(1::number, 3::number) i, generate_series(1,5,i) j;
...
--bug0000045
CREATE TABLE t0000045(a NUMBER(*));
SELECT attname, atttypid, atttypmod FROM pg_attribute
  WHERE attrelid = (SELECT oid FROM pg_class WHERE relname LIKE 't0000045') AND attname LIKE 'a';
DROP TABLE t0000045;

CREATE TABLE t0000045(a NUMBER(*, 10));
SELECT attname, atttypid, atttypmod FROM pg_attribute
  WHERE attrelid = (SELECT oid FROM pg_class WHERE relname LIKE 't0000045') AND attname LIKE 'a';
DROP TABLE t0000045;
...
/* Begin - bug0000457 */
SELECT BITAND(NULL::number, NULL::number);
SELECT BITAND(NULL, 1.1);
SELECT BITAND(1.1, NULL);
SELECT BITAND('', 1);
SELECT BITAND(' ', 1);
SELECT BITAND('1', 1);
SELECT BITAND('A', 1);
SELECT BITAND(1.1, 2.2);
SELECT BITAND(1.1::NUMERIC(3,1), 2.2::NUMERIC(3,1));
SELECT BITAND(1.1::NUMERIC(3,1), 2);
SELECT BITAND(1, 2.2::NUMERIC(3,1));
SELECT BITAND(1::int8, 2.2::NUMERIC(3,1));
SELECT BITAND(1::int8, 2.6::NUMERIC(3,1));
SELECT BITAND(-1, 2);-- 2
SELECT BITAND(-1, -1);-- -1
SELECT BITAND(-1.9, -2.1); -- -2
SELECT BITAND(1.9, 2.1); -- 2
SELECT BITAND(99999999999999999999999999999999999999, 1);--1
SELECT BITAND(99999999999999999999999999999999999999, 99999999999999999999999999999999999999);--9223372036854775807
SELECT BITAND(99999999999999999999999999999999999999, -99999999999999999999999999999999999999);--0
SELECT BITAND(99999999999999999999999999999999999999, -1);--9223372036854775807
SELECT BITAND(-99999999999999999999999999999999999999, 1);--0
SELECT BITAND(-99999999999999999999999999999999999999, 99999999999999999999999999999999999999);--0
SELECT BITAND(-99999999999999999999999999999999999999, -99999999999999999999999999999999999999);-- -9223372036854775808
SELECT BITAND(-99999999999999999999999999999999999999, -1);-- -9223372036854775808
/* End - bug0000457 */
